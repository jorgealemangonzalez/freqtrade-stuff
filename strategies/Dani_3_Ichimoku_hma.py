...
class Dani_3_Ichimoku_hma(IStrategy):
    ...
    def slow_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe4h = self.dp.get_pair_dataframe(pair=metadata['pair'], timeframe="4h")

        dataframe4h['hma105'] = ftt.hull_moving_average(dataframe4h, 105)
        dataframe = merge_informative_pair(dataframe, dataframe4h, self.timeframe, "4h", ffill=True)
        ...
        create_ichimoku(dataframe, conversion_line_period=1776, displacement=880, base_line_periods=880, laggin_span=880)
        ...
        dataframe['trending_over'] = (
                                         (dataframe['hma105_4h'] > dataframe['close'])
                                     ).astype('int') * 1
